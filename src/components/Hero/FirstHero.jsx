      ...
      <div className="border-l border-r border-gray-300 p-4">
        ...
        <div className="flex flex-col md:flex-row gap-2">
          ...
          <div className="md:w-1/2 p-2">
            ...
            className="text-xl md:text-2xl font-bold hover:underline cursor-pointer mb-3 ">
              {secondArticle?.title}
               <img
          ...
          <div className="md:w-1/2 p-2 flex flex-col justify-between">
            <div>
                <Link
              href={`${thirdArticle?.category?.slug}/${thirdArticle?.slug}`} className="text-xl md:text-2xl font-bold hover:underline cursor-pointer mb-4">
              {thirdArticle?.title}
              </Link>
              <hr className="h-px my-2 bg-gray-200 border-0" />
              <Link
              href={`${fourthArticle?.category?.slug}/${fourthArticle?.slug}`}
              className="text-xl md:text-2xl font-bold hover:underline cursor-pointer mt-4">
                {fourthArticle?.title}
              </Link>
